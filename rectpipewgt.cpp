...
RectPipeWgt::RectPipeWgt(QWidget *parent) : QWidget(parent)
{
    ...
    ph->setBuddy(phL);
    pH=new QLabel("<BODY><B>&H</B> - высота трубы, м;</BODY>");
    pH->setBuddy(pHL);
    pW=new QLabel("<BODY><B>&W</B> - ширина трубы, м;</BODY>");
    pW->setBuddy(pWL);
    ...
    toolTipStr<<" л/с. - расход;"<<" м/с. - скорость потока;"
            ...
            <<" м2. - площадь полного сечения трубы;"
           <<" м. - смоченный периметр;"<<" м. - периметр трубы;"
...
void RectPipeWgt::calc()
{
    ...
    if(phL->text().toDouble()>=pHL->text().toDouble())
    {
        ...
        pNoteLbl->setText("Уровень воды не может быть больше значения высоты трубы.");
        drawRect();
    }
    else
    {
...
void RectPipeWgt::slotPhLChanged()
{
    if(!(phL->text()==""))
    {
...
void RectPipeWgt::slotPWLChanged()
{
    if(!(pWL->text()==""))
    {
...
void RectPipeWgt::slotPnLChanged()
{
    if(!(pnL->text()==""))
    {
...
void RectPipeWgt::slotPiLChanged()
{
